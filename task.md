...
- [/] 核心功能: 书库与阅读器 (Library)
    ...
    - [x] **[NEW]** 分类书库功能 (Category Library)
        - [x] **双栏响应式布局搭建**
            - [x] 实现 Sidebar (左侧导航) + MainContent (右侧视图) 的父子结构
            - [x] 移动端支持侧边栏抽屉式切换
            - [x] 侧边栏宽度固定 (w-64)，背景色纯白，右侧框线分割
            - [x] 选中项高亮样式：背景色 indigo-50，文字色 indigo-700
        - [x] **新建分类功能**
            - [x] 生成唯一 categoryId，追加到分类列表
            - [x] 默认非系统分类 (isSystem: false)
            - [x] UI: 侧边栏底部虚线框按钮 "新建分类..."，点击触发模态框
        - [x] **重命名分类**
            - [x] 根据 ID 更新分类名称
            - [x] UI: 侧边栏 Item 悬停时显示 Edit 图标，点击弹出重命名模态框
        - [x] **删除分类 (带保护机制)**
            - [x] 删除分类前，遍历该分类下所有书籍，将 categoryId 重置为 null (归入"未分类")
            - [x] UI: 悬停显示 Trash 图标，弹出二次确认，提示"书籍将移动至未分类"
        - [x] **书籍导入/移动流程**
            - [x] 支持修改书籍的 categoryId，支持批量选择
            - [x] UI: 分类页 Header 的"导入书籍"按钮
            - [x] 弹窗展示非当前分类的书籍，选中样式为高亮边框+右上角打钩
            - [x] 每本书卡片下方展示"来自：[原分类名]"
        - [x] **智能视图：正在阅读**
            - [x] 基于书籍状态 (status === 'reading') 进行全局过滤
        - [x] **书籍删除功能**
            - [x] 书籍卡片长按触发 (Long Press)
            - [x] 二次确认弹窗 (UI)
            - [x] 联动删除文件与数据
            - [x] 侧边栏顶部固定位置，使用时钟图标
            - [x] 书籍封面有半透明黑色角标提示"阅读中"
        - [x] **数据结构扩展**
            - [x] Book 接口添加 categoryId 和 status 字段
            - [x] 新增 Category 接口 (id, name, isSystem)
            - [x] 实现 CategoryStore 服务
